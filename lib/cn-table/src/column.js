...
var _sfc_render = function render() {
  ...
  return _vm.children && _vm.children.length ? _c("el-table-column", { attrs: { "label": _vm.title } }, _vm._l(_vm.children, function(item, ind) {
    return _c("sub-column", { key: item.title + item.dataIndex + ind, attrs: { "width": item.width || "max-content", "minWidth": item.minWidth || "max-content", "ellipsis": item.ellipsis, "fixed": item.fixed, "title": item.title, "dataIndex": item.dataIndex, "status": item.status, "valueEnum": item.valueEnum, "children": item.children, "rend": item.render, "className": item.className, "autoCalcWidth": _vm.autoCalcWidth } });
