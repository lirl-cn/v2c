...
const index = require("../../cn-badge/src/index.js");
const _sfc_main = {
  components: {
    CnBadge: index
  },
...
var _sfc_render = function render() {
  ...
  return _vm.children && _vm.children.length ? _c("el-table-column", { attrs: { "label": _vm.title } }, _vm._l(_vm.children, function(item, index2) {
    return _c("sub-column", { key: item.title + item.dataIndex + index2, attrs: { "width": item.width || "max-content", "minWidth": item.minWidth || "max-content", "ellipsis": item.ellipsis, "fixed": item.fixed, "title": item.title, "dataIndex": item.dataIndex, "status": item.status, "valueEnum": item.valueEnum, "children": item.children, "rend": item.render, "className": item.className, "autoCalcWidth": _vm.autoCalcWidth } });
  }), 1) : _c("el-table-column", { key: _vm.title, attrs: { "column-key": _vm.dataIndex, "prop": _vm.dataIndex, "label": _vm.title, "width": _vm.width, "min-width": _vm.autoCalcWidth !== false ? _vm.title.length > 3 ? _vm.title.length / 4 * _vm.autoCalcWidth + 20 : 80 : _vm.minWidth, "show-overflow-tooltip": _vm.ellipsis, "fixed": _vm.fixed, "class-name": _vm.className }, scopedSlots: _vm._u([{ key: "default", fn: function({ row, $index }) {
    ...
    }, { "record": row, "text": _vm.renderText(row, _vm.dataIndex), "index": $index })];
  } }], null, true) });
};
