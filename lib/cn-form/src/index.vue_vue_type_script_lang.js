    ...
    data: {
      handler(val) {
        ...
        const rules = val.reduce((pre, cur) => {
          var _a;
          const key = cur.name || cur.dataIndex || cur.key(this).$set(
            this.formModel,
            key,
            key in initialValues ? initialValues[key] : (_a = cur.initialValue) != null ? _a : this.needDefaultValueArrayTypes.indexOf(cur.type) !== -1 ? [] : void 0
          );
