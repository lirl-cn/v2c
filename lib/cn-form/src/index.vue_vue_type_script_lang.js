    ...
    data: {
      handler(val) {
        ...
        const rules = val.reduce((pre, cur) => {
          ...
          const needDefaultValueArrayTypes = this.needDefaultValueArrayTypes || [];
          const isArrayType = needDefaultValueArrayTypes.indexOf(cur.type) !== -1;
          const defaultValue = isArrayType ? [] : void 0;
          const value = key in initialValues ? initialValues[key] : (_a = cur.initialValue) != null ? _a : defaultValue;
          this.$set(
            this.formModel,
            key,
            value
      ...
      return __async(this, arguments, function* (fields = []) {
        return new Promise((resolve, reject) => {
          this.$refs["formRef"].validateField(fields, (valid) => {
            if (valid) {
              if (fields.length === 1) {
                resolve(this.formModel[fields[0]]);
              } else {
                resolve(fields.reduce((pre, cur) => {
